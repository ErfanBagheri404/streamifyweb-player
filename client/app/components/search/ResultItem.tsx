...
export const ResultItem = memo<ResultItemProps>(
  ({ item, onPress, selectedSource }) => {
    ...
    const useWideMediaLayout = selectedSource === "mixed" || isYouTubeSource;
    ...
    const uploadedLabel =
      !isArtist && !hideViewsAndDateForYouTubeMusic && item.uploaded
        ? item.uploaded
        : undefined;
    const metaItems = [durationFormatted, viewsFormatted, uploadedLabel].filter(
    ...
    const imgWidth = isArtist ? 48 : useWideMediaLayout ? 192 : 96;
    const imgHeight = isArtist ? 48 : useWideMediaLayout ? 108 : 96;
          ...
          {!isArtist && (
            <div className="theme-muted flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
              {metaItems.map((metaItem, index) => (
                <React.Fragment key={`${item.id}-${metaItem}-${index}`}>
                  {index > 0 ? (
